...
namespace Game.Objects
{
    ...
    public class Actor
    {
        public enum AiState {Chasing,Caught,Wander,Figting,Stabled}
        ...
        public Actions.Enemy.EnemyType enemyType { get; set; }

        public List<Tuple<Items.Item.ItemType,float>> Drops { get; set; }
        public List<Actions.Enemy.EnemyType> Parents { get; set; }
        public List<Actions.Buff.BuffType> Buffs { get; set; }
        ...
        public void Update(Sprite2d sprite)
        {
            ...
            if (Util.Global.Sprites.Where(x => x.Actor != null && x.Actor.actorType == Objects.Actor.ActorType.Enemy && x.Position == Util.Global.DefaultPosition).Count() > 0)
            {
                Util.Global.Sprites.RemoveAll(x => x.Actor != null && x.Actor.actorType == Objects.Actor.ActorType.Enemy && x.Position == Util.Global.DefaultPosition);
            }
        ...
        public void UpdatePet(Sprite2d pet)
        {
            if (pet.Actor.aiState != AiState.Figting)
            {
                ...
                if (pet.Actor.aiState == AiState.Wander || pet.Actor.aiState == AiState.Stabled)
                {
                ...
                if (pet.Actor.aiState != AiState.Stabled)
                {
                ...
                else if (pet.Actor.aiState == AiState.Wander || pet.Actor.aiState == AiState.Stabled)
                {
                    ...
                    {
                        Vector2 seekPosition = 2 * Position - new Vector2(Hx, Hy);
                        pet.Orientation = TurnToFace(Position, seekPosition, pet.Orientation, 0.60f);
                    }
                }
                else
                {
                ...
                if (pet.Actor.aiState == AiState.Stabled)
                {
                    if (pet.ClipCheck())
                    {
                        pet.Position = pet.Actor.OriginalPosition;
                        pet.Orientation = pet.Orientation * -1.5f;
                    }
                    else
                    {
                        pet.Actor.OriginalPosition = pet.Position;
                    }
                }
